...
class RevSliderPluginUpdate {
	...
	public static function get_version(){
		$real_version = self::$framework->get_option('revslider_update_version', 1.0);
	...
	public static function set_version($set_to){
        ...
        self::$framework->update_option('revslider_update_version', $set_to);
    ...
    public static function get_v5_styles() {
		$v5 = array(
	...
	public static function update_css_styles(){
		...
		$css = new RevSliderCssParser(self::$framework, self::$query, self::$resource);
		$db = new RevSliderDB(self::$query, self::$resource);
	...
	public static function check_settings_table(){
		$wpdb = self::$query;
		if($wpdb->get_var("SHOW TABLES LIKE '".RevSliderGlobals::$table_settings."'") == self::$resource->getTableName(RevSliderGlobals::$table_settings)) {
			$result = $wpdb->get_row("SELECT `general` FROM ".RevSliderGlobals::$table_settings, Query::ARRAY_A);
			if(isset($result['general'])){
				self::$framework->update_option('revslider-global-settings', $result['general']);
			}
		}
	}
	...
	public static function move_template_slider(){
		...
		$sr = new RevSliderSlider(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts, self::$registerHelper);
		$sl = new RevSliderSlide(self::$framework, self::$query, self::$curl, self::$filesystem, self::$images, self::$resource, self::$googleFonts, self::$registerHelper);
	...
	public static function add_general_settings($sliders = false){
		...
		if(!empty($sliders) && is_array($sliders)){
			$fonts = self::$framework->get_option('tp-google-fonts', array());
