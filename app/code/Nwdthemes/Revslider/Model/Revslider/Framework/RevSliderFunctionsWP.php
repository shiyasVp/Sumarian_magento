...
class RevSliderFunctionsWP {
	...
	public function __construct(
        ...
        \Nwdthemes\Revslider\Helper\Products $products,
	...
	public static function isMultisite(){
		$isMultisite = self::$framework->is_multisite();
		return($isMultisite);
	}
	...
	public static function getPathUploads(){
		return self::$images->imageBaseDir();
	}
	...
	public static function getUrlUploads(){
		...
		if(self::isMultisite() == false){	//without multisite
			$baseUrl = self::$framework->content_url()."/";
		}
		else{	//for multisite
			$arrUploadData = self::$framework->wp_upload_dir();
    ...
    public static function isAdminUser(){
        return self::$framework->current_user_can('administrator');
    }
	...
	public static function import_media($file_url, $folder_name) {
		...
		$ul_dir = self::$framework->wp_upload_dir();
		$artDir = '';
		...
		//make sure the file actually exists, and not a folder
		if (strpos($filename, '.') !== false && @fclose(@fopen($file_url, "r"))) {
			...
			if($atc_id == false || $atc_id == NULL){
				...
				$artdata = array(
					'post_author' => 1, 
					'post_date' => self::$framework->current_time('mysql'),
					'post_date_gmt' => self::$framework->current_time('mysql'),
					'post_title' => $filename,
					'post_status' => 'inherit',
					'comment_status' => 'closed',
					'ping_status' => 'closed',
					'post_name' => self::$framework->sanitize_title_with_dashes(str_replace("_", "-", $filename)),
					'post_modified' => self::$framework->current_time('mysql'),
					'post_modified_gmt' => self::$framework->current_time('mysql'),
					'post_parent' => '',
				...
				$attach_id = self::$framework->wp_insert_attachment($artdata, $artDir.$folder_name.$filename);
			}else{
				$attach_id = $atc_id;
			}

			//generate metadata and thumbnails
			if($attach_data = self::$framework->wp_generate_attachment_metadata($attach_id, $saveDir)) self::$framework->wp_update_attachment_metadata($attach_id, $attach_data);
	...
	public static function registerWidget($widgetName){
		self::$framework->add_action('widgets_init', create_function('', 'return register_widget("'.$widgetName.'");'));
	}
	...
	public static function getImageUrlFromPath($pathImage){
        ...
        if(strpos($pathLower, "http://") !== false || strpos($pathLower, "https://") !== false || strpos($pathLower, "www.") === 0 || empty($pathImage)) {
			$pathImage = self::$images->imageFile($pathImage);
			return $pathImage;
		}
	...
	public static function getCategoriesAssoc($taxonomy = "category"){
		...
		$cats = self::$framework->get_categories($args);
	...
	public static function getPostTypeTitle($postType){
		...
		$objType = self::$framework->get_post_type_object($postType);
	...
	public static function getPostTypeTaxomonies($postType){
		$arrTaxonomies = self::$framework->get_object_taxonomies(array( 'post_type' => $postType ), 'objects');
	...
	public static function getPostTypesAssoc($arrPutToTop = array()){
		 ...
		 $arrCustomTypes = self::$framework->get_post_types(array('_builtin' => false));
    ...
    public static function getPostsByIDs($strIDs, $slider_id, $is_gal, $additional = array()){
		...
		$query = self::$framework->apply_filters('revslider_get_posts', $query, $slider_id);
		
		$arrPosts = self::$products->getProductsByQuery($query);

		//self::$framework->dmp($query);self::$framework->dmp("num posts: ".count($arrPosts));exit();
	...
	public static function getPostCategories($postID,$arrTax){
		...
		$arrCats = self::$framework->wp_get_post_terms( $postID, $arrTax);
	...
	public static function getPost($postID){
		$post = self::$framework->get_post($postID);
		if(empty($post))
	...
	public static function getUrlPostImage($postID,$size = self::THUMB_FULL){
		...
		$post_thumbnail_id = self::$framework->get_post_thumbnail_id( $postID );
		if(empty($post_thumbnail_id))
			return("");
		
		$arrImage = self::$images->wp_get_attachment_image_src($post_thumbnail_id,$size);
		if(empty($arrImage))
			return("");
	...
	public static function getPostThumbID($postID){
		$thumbID = self::$framework->get_post_thumbnail_id( $postID );
		return($thumbID);
	}
	...
	public static function getAttachmentImage($thumbID,$size = self::THUMB_FULL){
		...
		$arrImage = self::$images->wp_get_attachment_image_src($thumbID,$size);
		if(empty($arrImage))
			return(false);
	...
	public static function getExcerptById($postID, $limit=55){
        ...
        return self::$framework->apply_filters('revslider_getExcerptById', $excerpt, $post, $limit);
	}		
	...
	public static function getCategoriesByIDs($arrIDs,$strTax = null){			
		...
		$arrCats = self::$framework->get_categories( $args );
	...
	public static function getCategoriesHtmlList($catIDs,$strTax = null){
		...
		foreach ( $categories as $category ) {
			...
			$link = self::$framework->get_category_link($catID);
			$catName = $category["name"];
			
			if(!empty($link))
				$thelist .= '<a href="' . self::$framework->esc_url( $link ) . '" title="' . self::$framework->esc_attr( sprintf( __( "View all posts in %s"), $category["name"] ) ) . '" ' . $rel . '>' . $catName.'</a>';
			else
				$thelist .= $catName;
	...
	public static function getTagsHtmlList($postID){
		$tagList = self::$framework->get_the_tag_list("",",","",$postID);
		return($tagList);
	}
	...
	public static function convertPostDate($date, $with_time = false){
		if(empty($date))
			return($date);
		if($with_time){
			$date = self::$framework->date_i18n(self::$framework->get_option('date_format').' '.self::$framework->get_option('time_format'), strtotime($date));
		}else{
			$date = self::$framework->date_i18n(self::$framework->get_option('date_format'), strtotime($date));
		}
	...
	public static function getPathContent(){
        $pathContent = Framework::WP_CONTENT_DIR;
	...
	public static function hasShortcode($shortcode = '') {  
		...
		$post = self::$framework->get_post(get_the_ID());  
	...
	public static function get_image_id_by_url($image_url) {
		return self::$images->attachment_url_to_postid($image_url);
	}
	
	
    public static function update_option($handle, $value, $autoload = 'on'){ //on is on, false is 'off'

        if(!self::$framework->add_option($handle, $value, '', $autoload)){ //returns false if option is not existing
            self::$framework->delete_option($handle);
        }

        self::$framework->add_option($handle, $value, '', $autoload);
    }
