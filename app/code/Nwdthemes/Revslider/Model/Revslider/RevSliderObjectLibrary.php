...
class RevSliderObjectLibrary {
    ...
    public function _get_list($force = false){
        ...
        $last_check = $this->_framework->get_option('revslider-library-check');

        if($last_check == false){ //first time called
            $last_check = 1296001;
            $this->_framework->update_option('revslider-library-check',  time());
        }
        ...
        if(time() - $last_check > 1296000 || $force == true){ //30 days
            ...
            $count = 0;
            do {
                $url = $this->_revSliderLoadBalancer->get_url('library');
                $request = $this->_framework->wp_remote_post($url . '/' . $this->library_list, array(
                    'body' => $rattr
                ));
            ...
            if(!$this->_framework->is_wp_error($request)) {
                if($response = $this->_framework->maybe_unserialize($request['body'])) {
                    ...
                    if(is_array($library)) {
                        $this->_framework->update_option('rs-library', $library, false);
                    }
                }
            }
        }
    ...
    public function _get_object_thumb($object_handle, $type){
        ...
        if( ! file_exists($file) && $this->_filesystemHelper->wp_mkdir_p($upload_dir['basedir'].$path)) {
            ...
            if(!file_exists($file) || isset($temp['push_image'])){
                ...
                if($curl !== false){
                    ...
                    if($validated == 'false' && $type != 'thumb'){
                        ...
                        $code = ($validated == 'false') ? '' : $this->_framework->get_option('revslider-code', '');

                        $done = false;
                        $count = 0;
                        do {
                            $url = $this->_revSliderLoadBalancer->get_url('library');
                            $image_data = $this->_framework->wp_remote_post($url . '/' . $this->library_download, array(
                                'body' => array(
                                    'code' => urlencode($code),
                            ...
                            $response_code = $this->_framework->wp_remote_retrieve_response_code($image_data);
                            if ($response_code == 200) {
                                $done = true;
                            } else {
                                $this->_revSliderLoadBalancer->move_server_list();
    ...
    public function write_markup(){
        ...
        <div id="dialog_addobj" class="dialog-addobj" title="<?php echo __("Add Object Layer"); ?>" style="display:none">
                ...
                <div id="addobj-list-of-items">
                    <div id="addobj-dialog-header">
                        <div class="object_library_search_wrapper">
                            <input type="text" id="obj_library_search" placeholder="<?php echo __("Search for Objects..."); ?>" /><span id="obj_library_search_trigger"><i class="eg-icon-search"></i></span>
                        </div>
                        <div id="object_library_type_list_new">
                            <span id="obj_lib_main_cat_filt_all" data-value="all" class="obj_library_cats_filter"><?php echo __("ALL"); ?></span>
                            <span id="obj_lib_main_cat_filt_allimages" data-value="allimages" class="obj_library_cats_filter all_img_cat"><?php echo __("ALL IMAGES"); ?></span>
                            <span id="obj_lib_main_cat_filt_svg" data-value="svg" class="obj_library_cats_filter svg_cat"><?php echo __("SVG"); ?></span>
                            <span id="obj_lib_main_cat_filt_icon" data-value="icon" class="obj_library_cats_filter fonticon_cat"><?php echo __("ICON"); ?></span>
                            <span id="obj_lib_main_cat_filt_image" data-value="image" class="obj_library_cats_filter png_cat"><?php echo __("PNG"); ?></span>
                            <span id="obj_lib_main_cat_filt_bgimage" data-value="bgimage" class="obj_library_cats_filter jpg_cat"><?php echo __("JPG"); ?></span>
                        </div>
                        <div id="up-lic-ob-lib">
                            <div id="licence_obect_library"><i class="fa-icon-copyright"></i><?php echo __("License Info"); ?></div>
                            <div id="update_obect_library"><i class="eg-icon-arrows-ccw"></i><?php echo __("Update Object Library"); ?></div>
                        </div>
                        <div id="object-tag-list" class="object-tag-list"><span id="obj_library_cats_favorite" class="obj_library_cats" data-tag="favorite" data-image="true" data-bgimages="true" data-svg="true"><i class="fa-icon-star-o" style="margin-right: 5px;"></i><?php echo __("Favorite"); ?></span><span id="obj_library_cats_allico" class="obj_library_cats" data-tag="allicon" data-icon="true" data-bgimages="false" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo __("All Icons"); ?></span><span id="obj_library_cats_allpng" class="obj_library_cats" data-tag="allpng" data-image="true" data-bgimages="false" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo __("All PNG"); ?></span><span id="obj_library_cats_alljpg" class="obj_library_cats" data-tag="alljpg" data-image="false" data-bgimages="true" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo __("All JPG"); ?></span><span id="obj_library_cats_allsvg" class="obj_library_cats" data-tag="allsvg" data-image="false" data-bgimages="false" data-svg="true"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo __("All SVG"); ?></span></div>
                    </div>
            ...
            <div id="bg-vs-layer-wrapper">
                <span id="add_objimage_as_layer"><?php echo __("As Layer"); ?></span><span class="addthisasbg" id="obj-layer-bg-switcher"></span><span id="add_objimage_as_slidebg"><?php echo __("As Slide BG"); ?></span>
            </div>
    ...
    public function write_scripts(){
        ...
        <script>
            ...
            require(['jquery', 'themepunchTools', 'perfectScrollbar'], function(jQuery) {
                ...
                function createFontIcons() {
                    ...
                    if (sheets)
                        jQuery.each(sheets, function (index, sheet) {
                            var found = false,
                                markup = "";
                            if (sheet && sheet.href && sheet.href.indexOf("Nwdthemes_Revslider/public/assets/fonts/") >= 0) {
                                try {
    ...
    public function load_objects(){
        ...
        $online = $this->_framework->get_option('rs-library', array());
        if(!empty($online)){
            $obj['online'] = $online;
        }
        ...
        if(file_exists($image_path) && $imgsize = getimagesize($image_path)) {
            ...
            foreach($sizes as $size){
                ...
                if(!file_exists($upload_directory.$modified_file_name_without_ending.'.'.$file_ending) || $force) {
                    ...
                    $this->_imagesHelper->image_resize($image_path, $width, $height);
                }
            }
    ...
    public function get_correct_size_url($image_path, $imgres, $library_size = array()){
        ...
        if(!is_array($imgres)){
            ...
            $img_sizes = $this->_framework->get_intermediate_image_sizes();
    ...
    public function get_favorites(){
        return $this->_framework->get_option('rs_obj_favorites', array());
    }
    ...
    public function save_favorites($favourites){
        $this->_framework->update_option('rs_obj_favorites', $favourites);
    }
