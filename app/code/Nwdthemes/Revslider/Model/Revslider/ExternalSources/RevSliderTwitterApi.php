...
class RevSliderTwitterApi {
    ...
    public function __construct(
        ...
        if ( ! $this->bearer_token = $this->_framework->get_option( 'twitter_bearer_token' ) )
    ...
    private function get_bearer_token() {
            ...
            $response = $this->_framework->wp_remote_post( 'https://api.twitter.com/oauth2/token', $args );

            if ( $this->_framework->is_wp_error( $response ) || 200 != $response['response']['code'] )
                    return $this->bail( __( 'Can\'t get the bearer token, check your credentials', 'wp_twitter_api' ), $response );

            $result = json_decode( $response['body'] );

            $this->_framework->update_option( 'twitter_bearer_token', $result->access_token );
    ...
    public function query( $query, $query_args = array(), $stop = false ) {
            ...
            if ($this->query_args['cache'] > 0 && false !== ( $data = $this->_framework->get_transient( $transient_name ) ) )
            ...
            $response = $this->_framework->wp_remote_get( 'https://api.twitter.com/1.1/' . $this->query_args['type'] . '.json?' . $query, $args );
            if ( $this->_framework->is_wp_error( $response ) || 200 != $response['response']['code'] ){
            ...
            $this->_framework->set_transient( $transient_name, $response['body'], $this->query_args['cache'] );
    ...
    private function bail( $error_text, $error_object = '' ) {
            ...
            if ( $this->_framework->is_wp_error( $error_object ) ){
            ...
            $this->_framework->trigger_error( $error_text , E_USER_NOTICE );
